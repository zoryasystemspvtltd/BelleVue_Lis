...
{
    public class DxI800SerialCommand : SerialCommand
    {
        public DxI800SerialCommand(PortSettings settings)
            : base(settings)
        {
        ...
        {
            Logger.Logger.LogInstance.LogDebug("DXI800 CreateMessage method started. '{0}'", message);
            sInputMsg = "";
            ...
            {
                Logger.Logger.LogInstance.LogException("DXI800 CreateMessage method exception:", ex);
            }
            await Identify(formattedmessage);
            Logger.Logger.LogInstance.LogDebug("DXI800 CreateMessage method completed");
        }
        public override async Task SendOrderData(string sampleId)
        {
            try
            {
                Logger.Logger.LogInstance.LogDebug("DXI800 SendOrderData method started for SampleNo: " + sampleId);

                string datetime = DateTime.Now.AddMinutes(-30).ToString("yyyyMMddhhmmss");
                string specialchar = @"\^&";
                string headerSegment = $"1H|{specialchar}|||LIS|||||||P|1|{datetime}{Constants.vbCr}{Strings.Chr(3)}";
                ...
                if (testlist.Count() > 0)
                {
                    ...
                    var testname = "";
                    trailerSegment = $"4L|1|F{Constants.vbCr}{Strings.Chr(3)}";
                    ...
                    string patientSegment = $"2P|1|{patientId}|||{patientLastName}^{patientFirstName}^{patientMiddleName}|{Constants.vbCr}{Strings.Chr(3)}";
                    orderSegment += $"{testname}|R||||||A||||{specimen}|{Constants.vbCr}{Strings.Chr(3)}";

                    data[0] = Strings.Chr(5).ToString();
                    data[1] = headerSegment;
                    Logger.Logger.LogInstance.LogDebug("DXI800 Header Segment {0}", headerSegment);
                    data[2] = patientSegment;
                    Logger.Logger.LogInstance.LogDebug("DXI800 Patient Segment {0}", patientSegment);
                    data[3] = orderSegment;
                    Logger.Logger.LogInstance.LogDebug("DXI800 Order Segment {0}", orderSegment);
                    data[4] = trailerSegment;
                    Logger.Logger.LogInstance.LogDebug("DXI800 Trailer Segment {0}", trailerSegment);
                ...
                WriteToPort("" + (char)5);
                Logger.Logger.LogInstance.LogDebug("DXI800 SendOrderData method completed for SampleNo: " + sampleId);
            }
            catch (Exception ex)
            {
                Logger.Logger.LogInstance.LogException("DXI800 SendOrderData method exception:", ex);
            }
        }

        public override async Task Identify(string message)
        {
            Logger.Logger.LogInstance.LogDebug("DXI800 Identify method started");
            Logger.Logger.LogInstance.LogDebug("DXI800 Identify method Data:" + message);
            ...
            try
            {
                ...
                Logger.Logger.LogInstance.LogDebug("DXI800 Identify method completed");
            }
            catch (Exception ex)
            {
                Logger.Logger.LogInstance.LogException("DXI800 Identify method exception:", ex);
            }
        }
        ...
        public override async Task ParseMessage(string message, ArrayList sampleIdLst)
        {
            try
            {
                Logger.Logger.LogInstance.LogDebug("DXI800 ParseMessage method started");
                Logger.Logger.LogInstance.LogDebug("DXI800 ParseMessage method Data: " + message);
                ...
                for (int j = 0; j <= sampleIdLst.Count - 1; j++)
                {
                    ...
                    for (int index = 0; index <= record.Length - 1; index++)
                    {
                        ...
                        switch (field[0])
                        {
                            case "O":
                                {
                                    string[] sSpecimenId = field[2].Split('^');
                                    sampleNo = sSpecimenId[0];
                            ...
                            case "R":
                                {
                                    if (sampleNo == sampleIdLst[j].ToString())
                                    {
                                        ...
                                        string paramCode = parameter[3].Split('/')[0];
                                        if (paramCode != "")
                                        {
                                        ...
                                        Logger.Logger.LogInstance.LogDebug("DXI800 Result processed for SampleNo " + sampleNo + " and Parameter " + paramCode);
                    ...
                    Logger.Logger.LogInstance.LogDebug("DXI800 Result posted to API for SampleNo: " + testResult.SampleNo);
                    await LisContext.LisDOM.SaveTestResult(result);

                }
                Logger.Logger.LogInstance.LogDebug("DXI800 ParseMessage method completed");
            }
            catch (Exception ex)
            {
                Logger.Logger.LogInstance.LogException("ParseMessage method exception:", ex);
            }
        }
    }
}